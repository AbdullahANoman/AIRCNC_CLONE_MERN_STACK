...
export const findBooking = async (email) => {
  const url = `${import.meta.env.VITE_API_URL}/myBookings?email=${email}`;
...
// find manageBookings from host
export const findHostBookings = async (email) => {
  const url = `${import.meta.env.VITE_API_URL}/bookings/host?email=${email}`;
